...
def test_bundled_rules_are_valid_yaml_and_mapped():
    ...
    assert len(rules) == len({rule["id"] for rule in rules}) == 42
    assert sum(rule["languages"] == ["python"] for rule in rules) == 15
    assert sum(rule["languages"] == ["bash"] for rule in rules) == 27
