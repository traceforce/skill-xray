...
| `coverage._static_severity(reason, kind)` | `func StaticSeverity(reason, kind string) string` (`""` == None; `kind == ""` when no artifact) |
...
`_material_ledger_entry(entry, parsed)`: `phase != "static"` → true; reason not benign and `StaticSeverity(reason, kind) != ""`.
...
`_display(s)`: Python `unicode_escape` (§5.17). Text formats: `_print_one` (`package: %s`, `  seen=%d analyzed=%d skipped=%d coverage=%.1f%%`, `  %s %-40s %s%s`, `  SKIP  %-40s %s`), `_print_findings` (`  [%-8s] %-8s %-20s %s: %s%s`, loc `  L%d[:%d]` or `  @%d[+%d]`); `test_cli.py::test_text_finding_location_includes_column`, `::test_text_output_escapes_control_chars`, `::test_error_path_escapes_control_chars`.
...
14. **Crash-only exception names** (`check <module> failed: <Name>`, `capability-context-error: <Name>`, `correlation-error`, `disposition-error`, `llm-review-error`, `LLM adjudication pass failed (<Name>)`): Go recovers a panic and has no Python class name. Decision: use the recovered value's Go type name (`runtime.Error` → `RuntimeError`-like names are not attempted). These strings appear only when a check is broken; the parity harness treats any `check-error`/`*-error` as a scan-level defect to fix, not a string to match. `disposition-policy-error: ValueError` is deterministic and must be emitted literally.
