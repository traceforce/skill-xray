...
The binary was ported from a Python scanner and proved to produce the same findings, the same JSON and the same SARIF bytes on 2,346 fixture packages and the full benchmark test split before the Python code was retired; the detection contract it implements is written down in `docs/spec`.
...
3. Detection behaviour follows `docs/spec`. A change to detection lands with its tests and the matching spec update, and the benchmark and registry measurements in the report are re-run.
