...
The console carries no findings: one line per package, `VERDICT  seen=N analyzed=N cov=P%  package`, then `report: <path>`, and for `system-scan` a summary line of the package, blocking, with-findings, clean and discovery-exception counts. With `--llm`, a line starting `llm:` states the model calls made, whether the semantic pass ran, and how many reviews were sent or held back with the reason, so a run that never reached the model says so. Unicode control characters in a printed path are shown as escapes so a name cannot hide a line; the rest of the path prints as typed.
